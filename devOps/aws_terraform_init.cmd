@echo off
REM aws_terraform_init.cmd
REM Initializes AWS resources for Terraform backend (S3, DynamoDB, KMS)
REM Uses AWS CLI profile 'cna2' and region 'us-east-1'

set PROFILE=cna2
set REGION=us-east-1
for /f "tokens=*" %i in ('aws sts get-caller-identity --query Account --output text --profile "%PROFILE%"') do set ACCOUNT_ID=%i
set BUCKET=introspect2-tf-state-%ACCOUNT_ID%
set DYNAMO_TABLE=introspect2-tf-locks
set KMS_ALIAS=alias/introspect2-tf-key
...
aws s3api put-public-access-block --bucket "%BUCKET%" --public-access-block-configuration BlockPublicAcls=true,IgnorePublicAcls=true,BlockPublicPolicy=true,RestrictPublicBuckets=true --profile "%PROFILE%" --region "%REGION%"

echo Enabling default encryption (SSE-S3)
aws s3api put-bucket-encryption --bucket "%BUCKET%" --server-side-encryption-configuration "{\"Rules\":[{\"ApplyServerSideEncryptionByDefault\":{\"SSEAlgorithm\":\"AES256\"}}]}" --profile "%PROFILE%" --region "%REGION%"

echo Creating DynamoDB table for state locking: %DYNAMO_TABLE%
aws dynamodb create-table --table-name "%DYNAMO_TABLE%" --attribute-definitions AttributeName=LockID,AttributeType=S --key-schema AttributeName=LockID,KeyType=HASH --billing-mode PAY_PER_REQUEST --region "%REGION%" --profile "%PROFILE%"
